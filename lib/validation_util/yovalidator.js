...
const schemaGenerator = ({ clonedSchema }) => {
  if (!clonedSchema || typeof clonedSchema !== 'object') {
  ...
  // Handle array schemas (like your Liquibase schema)
  let schemaToProcess = clonedSchema;
  if (clonedSchema.type === 'array' && clonedSchema.items) {
    schemaToProcess = clonedSchema.items;
  }

  let clonedSchemaProps = schemaToProcess.properties;

  if (!clonedSchemaProps) {
