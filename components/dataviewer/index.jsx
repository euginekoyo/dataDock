...
const GridExample = ({ version }) => {
    ...
    const [columnDefs, setColumnDefs] = useState([
        ...
        },
        {
            ...
            pinned: 'left',
            cellRenderer: (props) => {
                if (!props.data?.validationData || props.data.validationData.length === 0) {
                    return null;
                }
                return (
                    <button
                        onClick={() => resolveRow(props.data, props.node)}
                        className="inline-flex items-center px-2 py-1 rounded-lg font-medium text-xs text-white bg-green-600 hover:bg-green-700"
                    >
    ...
    const cellCheckBySchema = useCallback(
        (field, value) => {
            console.log('cellCheckBySchema called:', {
                field,
                value,
    ...
    const nullValCheckBySchema = useCallback(
        (field, value) => {
            console.log('nullValCheckBySchema called:', {
                field,
                value,
                schema: Object.keys(schema).length,
            });
                ...
                for (let i = 0; i < rowCount; i++) {
                    ...
                    if (rowNode && data.data[rowNode.data._id]) {
                        rowNode.setDataValue(
                            'feedback',
                            JSON.stringify(
                                data.data[rowNode.data._id]?.feedback || data.data[rowNode.data._id]?.Feedback
                            )
                        );
                    }
                }
            ...
            if (enabled) {
                const dataSource = {
                    ...
                    getRows: async (params) => {
                        let url = `/api/meta?collection=${state.collection}&_start=${params.startRow}&_end=${
                            params.endRow
                        }`;
    ...
    const autofixUpdateDb = useCallback(
        (itemsToUpdate, params, label) => {
            ...
            axios
                .post('/api/autofix', dataToBeUpdated)
    ...
    useEffect(() => {
        ...
        const newColumnDefs = currentColumnDefs.map((elem) => {
            ...
            } else if (selectedErrorType === 'No selection' || elem.headerName === 'Row' || elem.headerName === 'Resolve') {
        ...
        if (gridRef?.current?.api && Object.keys(schema).length > 0) {
            ...
            if (Array.isArray(currentColumnDefs)) {
                const updatedColumnDefs = currentColumnDefs.map((colDef) => {
                    if (
                        colDef.field &&
                        colDef.field !== 'feedback' &&
                        colDef.field !== '_old' &&
                        colDef.field !== '_corrections' &&
                        colDef.headerName !== 'Row' &&
                        colDef.headerName !== 'Resolve'
                    ) {
                        return {
                            ...colDef,
        ...
        async (params) => {
            ...
            try {
                ...
                const dataSource = {
                    ...
                    getRows: async (params) => {
                        const url = `/api/meta?collection=${state.collection}&_start=${params.startRow}&_end=${
                            params.endRow
                        }`;
                        try {
                            const response = await fetch(url);
                        ...
                        <div className="p-4">
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="text-sm text-gray-600">Review and edit your data with real-time validation</p>
                                </div>
